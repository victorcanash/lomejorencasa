import { type ReactNode, useState, type FormEventHandler } from 'react'
...
const BaseForm = (props: FormBase) => {
  ...
  const handleSubmit = async (values: any, formikHelpers: FormikHelpers<any>) => {
    ...
    void formButtons?.submit?.onSubmit(values, formikHelpers, dirty)
  }

  const handleDelete = () => {
    if ((formButtons?.delete?.confirm?.enabled) ?? false) {
      handleDialog()
    } else {
    ...
    if (formField.type === FormFieldTypes.text ||
      ...
      return (
        <TextField
          ...
          error={(Boolean(getIn(props.touched, formField.name))) && Boolean(getIn(props.errors, formField.name))}
          helperText={(Boolean(getIn(props.touched, formField.name))) && getIn(props.errors, formField.name) as string}
        />
      )
    ...
    >
      ...
      >
        { props => (
          ...
          >
            <>
              { (formFieldGroups != null) && formFieldGroups.length > 0 &&
                <Grid
                  container
                ...
                >
                  { formFieldGroups.map((group, index) => (
                    ...
                    >
                      { ((group.avatarIcon != null) || (group.titleTxt != null) || (group.descriptionTxt != null)) &&
                        <Box
                          sx={{
                            display: 'flex',
                        ...
                        >
                          ...
                          { (group.descriptionTxt != null) &&
                            <Typography
                              component={formFieldGroups.length > 1 ? 'h3' : 'h2'}
                              variant="body1"
                              mb={((group.formFields != null) && group.formFields.length > 0) || (group.extraElements != null) ? 1 : 2}
                              mt={(group.titleTxt != null) ? 1 : undefined}
                              textAlign={group.descriptionTxt.textAlign}
                            >
                              <FormattedMessage
                      ...
                      { (group.formFields != null) && group.formFields.length > 0 &&
                        <Box mb={group.formFieldsMb ?? 2}>
              ...
              {(
                ((formButtons?.submit) != null) ||
                ((formButtons?.delete) != null) ||
                ((formButtons?.cancel) != null)
              ) &&
                <Grid
                  container
                >
              ...
              { (linksItems != null) && linksItems.length > 0 &&
                <Grid
                  container
                  sx={{
      ...
      { ((formButtons?.delete?.confirm?.enabled) ?? false) &&
        <ConfirmDialog
          open={openDialog}
