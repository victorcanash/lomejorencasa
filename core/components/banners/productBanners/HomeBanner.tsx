...
const HomeBanner = (props: HomeBannerProps) => {
  ...
  const typographySx = useMemo(() => {
    if (smallBreakpoint) {
      return ((themeElements.typography?.small) != null)
        ? convertElementToSx(themeElements.typography.small)
        : undefined
    }
    return ((themeElements.typography?.default) != null)
      ? convertElementToSx(themeElements.typography.default)
      : undefined
  }, [smallBreakpoint, themeElements.typography?.default, themeElements.typography?.small])

  const btnSx = useMemo(() => {
    return (themeElements.btn != null)
      ? convertElementToSx(themeElements.btn)
      : undefined
  }, [themeElements.btn])
